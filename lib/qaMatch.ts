...
/** Сохраняет ответ Gemini как готовый (answered) для похожего pending-вопроса или новый записи */
export async function saveGeminiAnswer(question: string, answer: string) {
  if (!isTursoConfigured()) return;
  ...
  const normalized = normalizeQuestion(question);
  const result = await db.execute("select * from questions where status = 'pending'");
  let matched: QAEntry | null = null;
  let bestScore = 0;
  ...
  if (matched) {
    await db.execute({
      sql: `update questions
        ...
        where id = ?`,
      args: [answer.trim(), now, matched.id],
    });
    return;
  }
  ...
  await db.execute({
    sql: `insert into questions
      (id, question, normalized_question, answer, status, asked_count, created_at, answered_at)
      values (?, ?, ?, ?, 'answered', 1, ?, ?)`,
    args: [crypto.randomUUID(), question.trim(), normalized, answer.trim(), now, now],
  });
}
